...
function tapEventListener(event) {
  ...
  if (_player.current_energy >= (4 + _player.player_level.level)) {
    ...
    if (_player.balance >= _levels[_player.player_level.level + 1]) {
      ...
      image.src = `https://cdn.umperium.com/static/character-level-${_player.player_level.level}.png.webp`;
      image.srcset = `https://cdn.umperium.com/static/character-level-${_player.player_level.level}@2x.png.webp 2x, https://cdn.umperium.com/static/character-level-${_player.player_level.level}@3x.png.webp 3x`;
...
async function showLevelModal() {
  ...
  console.log("Level threshold:", _levels[_player.player_level.level + 1]);

  const nextLevelThreshold = _levels[_player.player_level.level + 1];

  if (typeof nextLevelThreshold !== 'undefined') {
    const width = Math.floor(_player.balance / nextLevelThreshold * 100);
    progressEL.getElementsByClassName('progress-bar')[0].style.width = `${width}%`;
    progressEL.getElementsByClassName('progress-bar')[0].getElementsByClassName('progress--levelText')[0].innerHTML = `${formatBalance(_player.balance)} / ${formatBalance(nextLevelThreshold)}`;
  } else {
    // Если следующего уровня нет (например, для 7-го уровня)
    progressEL.getElementsByClassName('progress-bar')[0].style.width = `100%`;
    progressEL.getElementsByClassName('progress-bar')[0].getElementsByClassName('progress--levelText')[0].innerHTML = "Max level achieved";
  }
